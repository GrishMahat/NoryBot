  ...
  run: async (client, interaction) => {
    ...
    const handleProcess = async (members, description) => {
      ...
      const confirmEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `Are you ready to spread joy to ${members.size} ${description}?`
        )
        .addFields({
      ...
      const finalEmbed = new EmbedBuilder()
        ...
        .setTitle(
          cancelled
            ? 'Fun Fiesta Fizzled Out 😢'
            : 'Epic Fun Mission Complete! 🎊'
        )
        .setDescription(finalMessage)
        .setFooter({
          text: 'Remember, laughter is the best medicine (except for actual medicine)',
        })
        .setTimestamp();
    ...
    if (subcommand === 'user') {
      ...
      if (result.success) {
        ...
        await interaction.reply({
          content: `${funnyMessage} (Error: ${result.reason})`,
          ephemeral: true,
        });
      }
      ...
      await handleProcess(
        humanMembers,
        'unsuspecting victims... I mean, valued server members'
      );
    }
  },
};
