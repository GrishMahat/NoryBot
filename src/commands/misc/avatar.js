      ...
      if (style) {
        try {
          const avatarBuffer = await axios
            .get(userAvatar, { responseType: 'arraybuffer' })
          ...
          switch (style) {
            ...
            case 'sepia':
              ...
              break;
            case 'invert':
              const invertedImageData = ctx.getImageData(
                0,
                0,
                canvas.width,
                canvas.height
          ...
          styledAvatarAttachment = new AttachmentBuilder(styledAvatarBuffer, {
            name: 'styled_avatar.png',
          });
        } catch (error) {
          console.error('Error applying style to avatar:', error);
          await interaction.editReply({
            content:
              'Unable to apply the style to this avatar. It might be in an unsupported format or temporarily unavailable.',
            ephemeral: true,
          });
          return;
        }
