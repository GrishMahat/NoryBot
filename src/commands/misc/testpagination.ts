...
const testPagination: LocalCommand = {
	data: new SlashCommandBuilder()
        ...
        .addBooleanOption(option => 
            option.setName('jump')
                .setDescription('Enable Jump to Page button')
        )
        .addBooleanOption(option => 
            option.setName('stop')
                .setDescription('Enable Stop button')
        )
		.toJSON(),
	devOnly: false,
	run: async (client: Client, interaction: ChatInputCommandInteraction) => {
        ...
        for (let i = 0; i < 20; i++) {
            pages.push(
                new EmbedBuilder()
                    .setTitle(`Page ${i + 1}`)
                    .setDescription(`This is page number ${i + 1} of the pagination test.\n\nUse the controls below to navigate.`)
                    .setColor(colors[i % colors.length] as any)
                    .addFields(
                        { name: 'Random Data', value: Math.random().toString(36).substring(7) }
                    )
            );
        }
