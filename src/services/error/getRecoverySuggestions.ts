...
export default function getRecoverySuggestions(error: Error | DiscordAPIError | any): string[] {
	...
	if (typeof error.message === 'string') {
		...
		if (error.message.includes('EHOSTUNREACH')) {
			return ['Host unreachable. Check network connectivity and ensure the target server is online.'];
		}
	}

	// Default fallback suggestion
	return ['Check your bot code, network connectivity, and Discord API status. Consider implementing error logging for better debugging.'];
}
