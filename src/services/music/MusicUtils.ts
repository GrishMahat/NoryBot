...
function extractYoutubeId(url: string): string | null {
  const regex = /(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/watch\?v=|\/watch\?v=|\/embed\/)([a-zA-Z0-9_-]{11})/;
  const match = url.match(regex);
  return match ? match[1] : null;
}

function areMembersInSameVoiceChannel(member: GuildMember, botMember: GuildMember): boolean {
...
export { extractYoutubeId, areMembersInSameVoiceChannel, findFirstVoiceChannel, getVoiceChannel };
